export default async function handler(req, res) {
  ...
  if (!apiKey) {
    return res.status(500).json({ success: false, message: "API Key Vercel ਵਿੱਚ ਸੈੱਟ ਨਹੀਂ ਹੈ।" });
  }
...
3. ਪੜ੍ਹਾਈ ਵਾਲੇ ਸਵਾਲਾਂ ਦਾ ਜਵਾਬ ਸਪਸ਼ਟ, ਸਰਲ ਅਤੇ ਪੰਜਾਬੀ (ਗੁਰਮੁਖੀ) ਭਾਸ਼ਾ ਵਿੱਚ ਸਟੈਪ-ਬਾਈ-ਸਟੈਪ ਜਾਂ ਪੁਆਇੰਟਾਂ ਵਿੱਚ ਦਿਓ। ਸਹੀ ਉੱਤਰ ਸਭ ਤੋਂ ਪਹਿਲਾਂ ਰੱਖੋ।`;
  ...
  try {
    ...
    const answer = data.candidates?.[0]?.content?.parts?.[0]?.text || "ਜਵਾਬ ਤਿਆਰ ਕਰਨ ਵਿੱਚ ਸਮੱਸਿਆ ਆਈ, ਕਿਰਪਾ ਕਰਕੇ ਦੁਬਾਰਾ ਕੋਸ਼ਿਸ਼ ਕਰੋ।";

    return res.status(200).json({ success: true, answer });
  } catch (error) {
    return res.status(500).json({ success: false, message: "ਸਰਵਰ ਸਮੱਸਿਆ ਆਈ ਹੈ।" });
  }
}
